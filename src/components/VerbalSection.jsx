import React, { useState } from 'react';
import { CheckCircle2, Circle, BookOpen, AlertCircle } from 'lucide-react';
import { verbalData } from '../utils/studyData';

export default function VerbalSection({ completions, toggleCompletion }) {
  const [selectedTopic, setSelectedTopic] = useState(verbalData[0]);
  const [selectedAnswers, setSelectedAnswers] = useState({});

  const handleSelectOption = (questionIndex, optionIndex) => {
    const key = `\${selectedTopic.id}-\${questionIndex}`;
    if (selectedAnswers[key] !== undefined) return;
    
    setSelectedAnswers({
      ...selectedAnswers,
      [key]: optionIndex
    });
  };

  return (
    <div className="verbal-view">
      <div className="section-header">
        <h2>Verbal Trainer</h2>
        <p>ఇంగ్లీష్ గ్రామర్ మరియు కమ్యూనికేషన్ స్కిల్స్ - ఇంటర్వ్యూలు మరియు లిటన్ టెస్ట్ లలో రాణించడానికి.</p>
      </div>

      <div style={{ display: 'grid', gridTemplateColumns: '1fr 2fr', gap: '30px', alignItems: 'flex-start' }} className="dsa-grid">
        
        {/* Left Side: Topic list */}
        <div className="roadmap-list">
          {verbalData.map((topic) => {
            const isCompleted = completions[topic.id];
            return (
              <div 
                key={topic.id}
                onClick={() => setSelectedTopic(topic)}
                className={`roadmap-item \${selectedTopic.id === topic.id ? 'active' : ''}`}
                style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}
              >
                <div>
                  <h4 style={{ fontWeight: '600', fontSize: '1rem' }}>{topic.title}</h4>
                  <span style={{ fontSize: '0.8rem', color: 'var(--text-muted)' }}>
                    {topic.quiz.length} Practice Questions
                  </span>
                </div>
                <button 
                  onClick={(e) => {
                    e.stopPropagation();
                    toggleCompletion(topic.id);
                  }}
                  style={{ background: 'transparent', border: 'none', cursor: 'pointer', color: isCompleted ? 'var(--success)' : 'var(--text-muted)' }}
                >
                  {isCompleted ? <CheckCircle2 size={24} /> : <Circle size={24} />}
                </button>
              </div>
            );
          })}
        </div>

        {/* Right Side: Topic details */}
        <div className="card">
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px' }}>
            <h3 style={{ fontSize: '1.4rem' }}>{selectedTopic.title} Rules</h3>
            <button 
              onClick={() => toggleCompletion(selectedTopic.id)}
              className="nav-btn"
              style={{ padding: '8px 12px', border: '1px solid var(--border-color)', borderRadius: '8px', display: 'flex', gap: '8px', alignItems: 'center' }}
            >
              {completions[selectedTopic.id] ? (
                <>
                  <CheckCircle2 size={16} style={{ color: 'var(--success)' }} />
                  <span style={{ color: 'var(--success)', fontSize: '0.85rem' }}>Completed</span>
                </>
              ) : (
                <>
                  <Circle size={16} />
                  <span style={{ fontSize: '0.85rem' }}>Mark Completed</span>
                </>
              )}
            </button>
          </div>

          {/* Grammar Rules */}
          <div style={{ background: 'rgba(255,255,255,0.015)', padding: '18px', borderRadius: '12px', marginBottom: '25px', border: '1px solid var(--border-color)' }}>
            <h4 style={{ fontSize: '1rem', color: '#ec4899', fontWeight: '600', marginBottom: '12px', display: 'flex', alignItems: 'center', gap: '8px' }}>
              <BookOpen size={18} /> Core Grammar Rules & Examples
            </h4>
            <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
              {selectedTopic.rules.map((rule, i) => (
                <div key={i} style={{ borderBottom: i < selectedTopic.rules.length - 1 ? '1px solid var(--border-color)' : 'none', paddingBottom: '10px' }}>
                  <p style={{ fontSize: '0.92rem', color: 'var(--text-main)', fontWeight: '500' }}>{rule}</p>
                </div>
              ))}
            </div>
          </div>

          {/* Practice Quiz */}
          <div>
            <h4 style={{ fontSize: '1.1rem', marginBottom: '15px', display: 'flex', alignItems: 'center', gap: '8px' }}>
              <AlertCircle size={20} style={{ color: '#ec4899' }} /> Grammar Practice Quiz
            </h4>

            {selectedTopic.quiz.map((q, qIdx) => {
              const key = `\${selectedTopic.id}-\${qIdx}`;
              const userAnswerIndex = selectedAnswers[key];
              const isAnswered = userAnswerIndex !== undefined;

              return (
                <div key={qIdx} className="quiz-panel" style={{ marginBottom: '24px' }}>
                  <p style={{ fontWeight: '600', fontSize: '0.95rem', marginBottom: '14px', color: 'var(--text-main)' }}>
                    Q{qIdx + 1}: {q.question}
                  </p>
                  
                  <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                    {q.options.map((opt, oIdx) => {
                      let optionClass = '';
                      if (isAnswered) {
                        if (oIdx === q.answer) {
                          optionClass = 'correct';
                        } else if (oIdx === userAnswerIndex) {
                          optionClass = 'incorrect';
                        }
                      }
                      
                      return (
                        <div 
                          key={oIdx}
                          onClick={() => handleSelectOption(qIdx, oIdx)}
                          className={`quiz-option \${optionClass}`}
                          style={{ margin: 0 }}
                        >
                          <span>{opt}</span>
                          {isAnswered && oIdx === q.answer && <span style={{ fontSize: '0.8rem', fontWeight: '700' }}>✓ Correct</span>}
                          {isAnswered && oIdx === userAnswerIndex && oIdx !== q.answer && <span style={{ fontSize: '0.8rem', fontWeight: '700' }}>✗ Selected</span>}
                        </div>
                      );
                    })}
                  </div>

                  {/* Show explanation after answer is selected */}
                  {isAnswered && (
                    <div className="explanation-box" style={{ borderLeftColor: '#ec4899', background: 'rgba(236, 72, 153, 0.03)' }}>
                      <strong>Grammar Explanation (పరిష్కారం):</strong>
                      <p style={{ marginTop: '6px', fontSize: '0.88rem' }}>{q.explanation}</p>
                    </div>
                  )}
                </div>
              );
            })}
          </div>
        </div>
      </div>
    </div>
  );
}
